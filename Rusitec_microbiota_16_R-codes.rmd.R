...
#Safe the OTU-table as a csv.-Excel-file
ps1_otu <- as.data.frame(otu_table(ps1_relabund))
ps1_taxa <- as.data.frame(tax_table(ps1_relabund))
ps1_.df <- cbind(ps1_taxa,ps1_otu)
write.table(ps1_.df,file="OTU-table.csv")

#Subset only LAB (liquid-associated microbes) and SAB (solid-associated microbes) for the respective time points (B = early and C = late) as relative abundances
...
ps1.adiv <- estimate_richness(ps1_C_LAB, measures = c("Chao1", "Shannon", "Observed", "InvSimpson"))
ps1.metadata <- as(sample_data(ps1_C_LAB), "data.frame")
head(ps1.metadata)
...
rel_distance <- distance(ps1_C_LAB, "wunifrac")
adonis(rel_distance ~ SA, as(sample_data(ps1_C_LAB), "data.frame"))

#Test beta-diversity for significance by adonis for unweighted unifrac
#Insert the respective phyloseq object (e.g. ps1_LAB) and the variable/treatment of interest, no p-value adjustment due to multiple testing is needed
rel_distance <- distance(ps1_C_SAB, "uunifrac")
adonis(rel_distance ~ SA, as(sample_data(ps1_C_SAB), "data.frame"))
